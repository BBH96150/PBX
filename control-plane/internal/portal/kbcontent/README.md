# Help Center

Welcome to the help center for your cloud phone system. Sign in to the admin portal at **https://pbx.tendpos.com/admin**.

New here? Start with [Welcome: how your phone system is organized](getting-started-overview.md), then run through the [First-time setup checklist](first-time-setup.md).

## Getting Started

- [Welcome: how your phone system is organized](getting-started-overview.md)
- [First-time setup checklist](first-time-setup.md)
- [Manage your own phone (self-service)](self-service-portal.md)

## Phones & Extensions

- [Create and manage extensions](extensions-create-and-manage.md)
- [Add and provision desk phones](devices-provision-desk-phones.md)
- [Connect a softphone with SIP credentials](softphone-sip-credentials.md)
- [Use the web softphone](softphone-web.md)
- [Connect a carrier (SIP trunk)](trunks-connect-carrier.md)

## Calling Features

- [Route inbound phone numbers (DIDs)](numbers-route-dids.md)
- [Outbound calling routes](numbers-outbound-routing.md)
- [Business hours and after-hours routing](numbers-business-hours.md)
- [Ring groups](calling-ring-groups.md)
- [Call queues](calling-call-queues.md)
- [IVR menus (auto-attendant)](calling-ivr-menus.md)
- [Call forwarding, do-not-disturb, and recording](calling-forwarding-and-dnd.md)
- [Set up voicemail and voicemail-to-email](voicemail-setup.md)
- [Music on hold](calling-music-on-hold.md)

## Paging & Broadcast

- [Paging groups (intercom / PTT)](paging-groups.md)
- [The Broadcast app: voice blasts and live push-to-talk](paging-broadcast-app.md)

## Messaging & Directory

- [SMS messages (preview)](messaging-sms.md)
- [Directory / contacts](messaging-directory.md)

## Reports & History

- [Call history and recordings](reports-call-history.md)
- [Reports and the live call view](reports-live-and-reports.md)
- [The live queue board](reports-queue-board.md)

## Admin & Security

- [Invite your team and set roles](admin-team-and-roles.md)
- [Your account: profile, password, and security](admin-account-and-password.md)
- [Two-factor authentication (2FA)](admin-two-factor.md)
- [Manage active sessions](admin-sessions.md)
- [Single sign-on (OIDC and SAML)](admin-sso.md)
- [Audit log](admin-audit-log.md)

## Integrations & API

- [API keys](integrations-api-keys.md)
- [Outbound webhooks](integrations-webhooks.md)
- [Trunk-down alerts and the daily digest](integrations-alerts-and-digest.md)

## Troubleshooting & FAQ

- [Troubleshooting & FAQ](troubleshooting-faq.md)

## Reference

- [Glossary of phone-system terms](glossary.md)

...
- **Prod compose is partly hand-maintained.** The box's base `docker-compose.yml` has diverged from this repo's (it uses `image:` not `build:`, file-mounted SAML certs, an always-on Caddy, a different redis binding, and predates `SIP_DOMAIN_SUFFIX`/`fs-gateways-init`). The CI deploy only syncs `db/migrations/` and an ADDITIVE `docker-compose.prod.yml` (landed as `docker-compose.override.yml`) — it does **not** sync the base, since a blind rsync would break prod. Reconciling the base into repo-controlled `base + prod-override` files is a future hardening task.
...
- **Voicemail recordings** live on the FreeSWITCH host under `storage_dir`; the `freeswitch_storage` volume is now shared (rw to FS, ro to control-plane) so the portal inbox can stream them and VM-to-email can attach them. If the mount is ever absent, email falls back to a "see FS host at <path>" note and the inbox stream returns 404.
...
- **Per-tenant custom MoH** — a tenant can set `moh_url` (any FS-resolvable source); the dialplan sets `hold_music` on internal + inbound-extension calls when present. Falls back to the default `local_stream://moh` when unset. Per-queue MoH is also editable on the queue page.
- **Portal does not yet have**: field-level *edit* for most top-level entities (delete + enable/disable now exist). Read shows everything; writes cover: tenants, extensions, devices (+ line binding, delete), ring groups (+ members, disable/delete), IVRs (+ menu options, disable/delete), queues (+ agents, disable/delete), extension features (DND/CF/VM/recording), voicemail box create + message inbox (play/download/delete), DIDs (create/edit/delete with all five destination kinds + after-hours routing), business-hours schedules (weekly periods + holiday overrides), outbound routes, API tokens.

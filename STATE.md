...
- **Portal does not yet have**: field-level *edit* for most top-level entities (delete + enable/disable now exist). Read shows everything; writes cover: tenants, extensions, devices (+ line binding, delete), ring groups (+ members, disable/delete), IVRs (+ menu options, disable/delete), queues (+ agents, disable/delete), extension features (DND/CF/VM/recording), voicemail box create + message inbox (play/download/delete), DIDs (create/edit/delete with all five destination kinds), outbound routes, API tokens.

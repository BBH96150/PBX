...
- **Portal does not yet have**: edit/delete for most top-level entities, device-line binding. Read shows everything; writes cover: tenants, extensions, devices, ring groups (+ members), IVRs (+ menu options), queues (+ agents), extension features (DND/CF/VM/recording), voicemail box create + message inbox (play/download/delete), DIDs (create/edit/delete with all five destination kinds), outbound routes, API tokens.
